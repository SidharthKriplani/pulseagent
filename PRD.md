...
| "Your citation precision is 0.26 — that seems low. Is it?" | Evidence honesty | "Yes it's low, I'm working on it" | Define the metric: len(verified)/3 averaged across 200 queries including 88 ABSTAINs with precision=0; among answered queries it's ~0.46; also clarify this is NLI-pass rate, not ground-truth article overlap |
| "Why does the system have a 44% abstain rate? Is that a problem?" | Product judgment | "It means the system is not good enough" | Abstain rate is a design decision, not a failure metric; it means the system refused to cite when it could not verify; the correct question is whether the ANSWER_WITH_CITATION subset has good citation quality, and whether the abstain threshold is right for the product context |
...
| "How would you improve this system?" | Engineering vision | "Add more data" | Four concrete priorities: (1) paragraph-level retrieval index instead of article-level to improve NLI signal; (2) eval with actual expert-written questions (not titles) against ground-truth article_ids; (3) retrieval ablation (BM25-only vs dense-only vs hybrid); (4) NLI threshold sensitivity curve to find optimal tradeoff |
